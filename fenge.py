...
if __name__ == '__main__':
    img_path = r"H:\CL\1\2.tif"
    temp_path = r"H:\CL\shp\2\\"
    start = time.time()
    ...
    # segments_qucikshift = quickshift(temp,kernel_size=100,max_dist=100,ratio=10,sigma=100)
    # segments_watershed = watershed(temp)
    # segments_fe = felzenszwalb(temp,scale = 1,sigma = 4)
    segments_slic = slic(temp, n_segments=2000, compactness=50,  sigma=1) # 使用slic对遥感图像进行超像素分割
    mark0 = mark_boundaries(temp,segments_slic) # 返回带有带有边界的标签图像
    # mark0 = mark_boundaries(temp, segments_qucikshift)
    # mark0 = mark_boundaries(temp,segments_fe)
    # segments_quick = quickshift(temp, kernel_size=3, max_dist=6, ratio=0.5)

    # mark0 = mark_boundaries(temp, segments_slic)
    ...
    # g = graph.rag_mean_color(temp, segments_slic) # 给定原始图像，与分割后的图像，返回该方法构造的相应区域邻接图
    # g = graph.rag_mean_color(temp,segments_qucikshift)
    ...
    # labels2 = graph.merge_hierarchical(segments_qucikshift, g, thresh=5,
    #                                    rag_copy=False,
    #                                    in_place_merge=True,
    #                                    merge_func=merge_mean_color,
